...
// ── EXACT LANDMARKS ──────────────────────────────────────────
const LANDMARKS = {
  'monumento': 'Sa tabi ng SM Grand Central o MCU',
  'recto': 'Sa kahabaan ng C.M. Recto Ave o Isetann',
  'cubao': 'Sa Aurora Blvd o Araneta City Jeepney Terminal',
  'divisoria': 'Sa kalye ng Recto Ave o Juan Luna St.',
  'quiapo': 'Malapit sa Quiapo Church o Quezon Blvd',
  'pasay': 'Sa Pasay Rotonda (EDSA-Taft) o LRT-1 Libertad',
  'makati': 'Sa Ayala Ave loading zone o Makati Ave',
  'bgc': 'Sa Market! Market! transport terminal',
  'moa': 'Sa MOA Transport Terminal (sa tapat ng bay)',
  'fairview': 'Sa Commonwealth Ave o SM Fairview Terminal',
  'novaliches': 'Sa Novaliches Bayan o Robinsons Nova Stop',
  'alabang': 'Sa Starmall Alabang o South Station Terminal',
  'antipolo': 'Malapit sa Antipolo Simbahan o Shopwise',
  'marikina': 'Sa Marikina Sports Center o Bayan',
  'ortigas': 'Malapit sa Robinsons Galleria o Megamall Bay',
  'manila': 'Sa pinakamalapit na kanto ng Taft Ave o P. Burgos'
};
const getLandmark = (name) => LANDMARKS[name?.toLowerCase()] || 'Karaniwan nasa tabi ng kalsada o sa may palengke';
const getBusLandmark = (name) => LANDMARKS[name?.toLowerCase()] || 'Karaniwan nasa EDSA, major road, o transport terminal';
const EDSA_AREAS = ['monumento', 'north avenue', 'quezon avenue', 'cubao', 'shaw boulevard', 'ortigas', 'mandaluyong', 'makati', 'ayala', 'pasay', 'moa', 'baclaran', 'bgc', 'caloocan'];

// ── MAJOR TRANSFER HUBS (para sa 2-jeepney routes) ────────────
const JEEP_HUBS = [
  { name:'Cubao',       lat:14.6192,lng:121.0509, icon:'🏙️', sign1:'"CUBAO"',       sign2:'"CUBAO"' },
  { name:'Quiapo',      lat:14.5986,lng:120.9822, icon:'⛪',  sign1:'"QUIAPO"',      sign2:'"QUIAPO"' },
  { name:'Monumento',   lat:14.6538,lng:120.9832, icon:'🗽',  sign1:'"MONUMENTO"',   sign2:'"MONUMENTO"' },
  { name:'Divisoria',   lat:14.5995,lng:120.9723, icon:'🛍️', sign1:'"DIVISORIA"',   sign2:'"DIVISORIA"' },
  { name:'Ortigas',     lat:14.5870,lng:121.0562, icon:'🏢',  sign1:'"ORTIGAS"',     sign2:'"ORTIGAS"' },
  { name:'Makati',      lat:14.5547,lng:121.0244, icon:'🏢',  sign1:'"AYALA"',       sign2:'"AYALA"' },
  { name:'Pasay',       lat:14.5389,lng:121.0000, icon:'✈️',  sign1:'"PASAY"',       sign2:'"PASAY"' },
  { name:'Mandaluyong', lat:14.5794,lng:121.0359, icon:'🏙️', sign1:'"MANDALUYONG"', sign2:'"MANDALUYONG"' },
  { name:'Caloocan',    lat:14.6500,lng:120.9672, icon:'🏙️', sign1:'"CALOOCAN"',    sign2:'"CALOOCAN"' },
  { name:'Marikina',    lat:14.6494,lng:121.1028, icon:'👟',  sign1:'"MARIKINA"',    sign2:'"MARIKINA"' },
];

// ── 2026 FARE CONSTANTS ────────────────────────────────────────
const ROAD_MODES = [
  { key:'trad',  icon:'🚌', name:'Tradisyonal na Jeepney',  note:'Hindi aircon · bawat araw gamit',        base:13, baseKm:4, perKm:2.00, colorClass:'c-green',  tag:'tag-legal',   tagText:'✓ Legal' },
  { key:'mod',   icon:'🚐', name:'Modern / E-Jeepney',      note:'May aircon · mas bago',                  base:15, baseKm:4, perKm:2.40, colorClass:'c-blue',   tag:'tag-legal',   tagText:'✓ Legal' },
  ...
  { key:'carousel', icon:'🚍', name:'EDSA Carousel',        note:'Aircon · may libre at dedikadong busway',base:15, baseKm:5, perKm:2.20, colorClass:'c-green',  tag:'tag-best',    tagText:'⚡ Mabilis' },
...
function roadDist(a, b){ return Math.round(hav(a,b)*1.22*10)/10; }
function calcFare(m, d) {
  if (m.perKm === null) return m.base;
  if (d <= m.baseKm) return m.base;
  return m.base + Math.floor(d - m.baseKm) * m.perKm;
}
function calcTotalRoadFare(m, from, to) {
  if (m.key === 'p2p') return m.base;
  const hub = findJeepHub(from, to);
  if (hub) {
    return calcFare(m, roadDist(from, hub)) + calcFare(m, roadDist(hub, to));
  }
  return calcFare(m, roadDist(from, to));
}
...
function nearestStation(area, radius=15.0) {
...
function getConnectorStep(area, station, type) {
  if (station.walkKm <= 1.2) {
    return {
      step: { icon:'🚶', action: type==='from' ? `Lumakad papunta sa ${station.name} Station` : `Lumabas at lakad papunta sa ${area.name}`, detail:`Mga ${walkTime(station.walkKm)} minuto lakad`, cost:null },
      time: walkTime(station.walkKm),
      fare: 0
    };
  }
  const jFare = calcFare(ROAD_MODES[0], station.walkKm);
  const jTime = Math.round(station.walkKm * 5 + 8);
  const action = type === 'from' ? `Sumakay ng Jeep papuntang ${station.name} Station` : `Sumakay ng Jeep papuntang ${area.name}`;
  return {
    step: { icon:'🚏', action, detail:`~${jTime} minuto · Bayad: ₱${jFare.toFixed(2)}`, cost:`₱${jFare.toFixed(2)}` },
    time: jTime,
    fare: jFare
  };
}

// Find if a transfer hub lies along the route (for 2-jeepney calculations)
function findJeepHub(from, to) {
  const directDist = hav(from, to);
  if (directDist < 6) return null; // Short routes — single jeepney fine

  let bestHub = null;
  let bestDetour = 1.35; // max 35% longer via hub = still "on the way"

  for (const hub of JEEP_HUBS) {
    const dFrom = hav(from, hub);
    const dTo   = hav(hub, to);
    // Skip if hub is the same as origin/destination
    if (dFrom < 0.8 || dTo < 0.8) continue;
    const detour = (dFrom + dTo) / directDist;
    if (detour < bestDetour) { bestDetour = detour; bestHub = { ...hub, dFrom, dTo }; }
  }
  return bestHub;
}
...
function generateDirections(fromArea, toArea, dist) {
  ...
  if (fromSt && toSt && dist >= 3.5) {
    ...
    if (sameLine && different) {
      const cFrom = getConnectorStep(fromArea, fromSt, 'from');
      const cTo   = getConnectorStep(toArea, toSt, 'to');
      const railF   = railFareBetween(fromSt, toSt);
      const fare    = railF + cFrom.fare + cTo.fare;
      const trainT  = travelTimeMin(fromSt, toSt);
      const totalT  = trainT + cFrom.time + cTo.time;
      const lineName= LINE_LABEL[fromSt.line];
      const dir     = fromSt.order < toSt.order ? 'papuntang dulo' : 'pabalik sa simula';

      const mixPrefix = cFrom.fare > 0 ? 'Jeep + ' : '';
      const mixSuffix = cTo.fare > 0 ? ' + Jeep' : '';

      const steps = [];
      if (fromSt.walkKm > 0.15) steps.push(cFrom.step);
      steps.push({ icon:'🎟️', action:`Bumili ng ticket o i-tap ang Beep Card`, detail:`Bayad: ₱${railF} · 50% diskuwento na ipinatupad ngayong 2026`, cost:`₱${railF}` });
      ...
      if (toSt.walkKm > 0.15) steps.push(cTo.step);

      const stRail = Math.round(railF * 0.8);
      const stTotal = stRail + (cFrom.fare * 0.8) + (cTo.fare * 0.8);

      options.push({
        rank:1, tag:'green', tagLabel: mixPrefix ? 'Mabilis (Mixed)' : 'Pinaka-Mura at Mabilis', type:`${mixPrefix}${lineName}${mixSuffix}`,
        price:`₱${fare.toFixed(2)}`, priceClass:'green',
        studentPrice: `₱${stTotal.toFixed(2)}`,
    ...
    if (!sameLine && fromSt.line==='MRT3' && toSt.line==='LRT1') {
      ...
      if (mrt3Taft && lrt1Edsa) {
        ...
        const totalFare = (fare1||0)+(fare2||0) + cFrom.fare + cTo.fare;
        const train1T = travelTimeMin(fromSt, mrt3Taft);
        const train2T = travelTimeMin(lrt1Edsa, toSt);
        const totalT  = train1T + 8 + train2T + cFrom.time + cTo.time;
        ...
        const stRail = Math.round(fare1 * 0.8) + Math.round(fare2 * 0.8);
        const stTotal = stRail + (cFrom.fare * 0.8) + (cTo.fare * 0.8);

        options.push({
          rank:1, tag:'green', tagLabel: mixPrefix ? 'Rail Transfer (Mixed)' : 'Rail Transfer', type:`${mixPrefix}MRT-3 + LRT-1${mixSuffix}`,
          price:`₱${totalFare.toFixed(2)}`, priceClass:'green',
          studentPrice: `₱${stTotal.toFixed(2)}`,
          time:`~${totalT} minuto`, steps
        });
      }
    ...
    if (!sameLine && (fromSt.line==='LRT2'||toSt.line==='LRT2') && (fromSt.line==='LRT1'||toSt.line==='LRT1')) {
      ...
      if (lrt2Recto && lrt1DorJose) {
        ...
        const totalFare = fare1 + fare2 + cFrom.fare + cTo.fare;
        const totalT    = t1+10+t2+cFrom.time+cTo.time;

        const mixPrefix = cFrom.fare > 0 ? 'Jeep + ' : '';
        const mixSuffix = cTo.fare > 0 ? ' + Jeep' : '';

        const isLrt2First = fromSt.line === 'LRT2';
        const steps = [];
        if (fromSt.walkKm>0.15) steps.push(cFrom.step);
        ...
        if (isLrt2First) {
          ...
          steps.push({ icon:'🎟️', action:'Bumili ng ticket / tap Beep Card (LRT-1)', detail:`Bayad: ₱${fare2}`, cost:`₱${fare2}` });
          steps.push({ icon:'🟢', action:`Sumakay ng LRT-1 papuntang Doroteo Jose Station`, detail:`Bumaba sa Doroteo Jose Station · ~${t2} minuto`, cost:null });
          steps.push({ icon:'🔀', action:'Lumabas at lakad papunta sa Recto (LRT-2)', detail:'~7–10 minuto · tumawid sa connecting bridge papuntang Recto Station', cost:null });
          steps.push({ icon:'🎟️', action:'Bumili ng bagong ticket (LRT-2)', detail:`Bayad: ₱${fare1}`, cost:`₱${fare1}` });
          steps.push({ icon:'🔵', action:`Sumakay ng LRT-2 papuntang ${lrt2Side.name}`, detail:`~${t1} minuto`, cost:null });
        }
        ...
        options.push({
          ...
          tagLabel: mixPrefix ? 'Rail Transfer (Mixed)' : 'Rail Transfer',
          type: isLrt2First ? `${mixPrefix}LRT-2 + LRT-1${mixSuffix}` : `${mixPrefix}LRT-1 + LRT-2${mixSuffix}`,
          price:`₱${totalFare.toFixed(2)}`, priceClass: options.length===0?'green':'blue',
          studentPrice: `₱${stTotal.toFixed(2)}`,
          time:`~${totalT} minuto`, steps
        });
      }
  ...
  // ── OPTION B: Jeepney — with 2-ride transfer detection ────────
  const hub = findJeepHub(fromArea, toArea);
  ...
  let tradFare, jeepTime, jeepSteps;

  if (hub) {
    // ── 2 jeepney rides with transfer at hub ──
    const d1 = roadDist(fromArea, hub);
    const d2 = roadDist(hub, toArea);
    const fare1 = calcFare(ROAD_MODES[0], d1);
    const fare2 = calcFare(ROAD_MODES[0], d2);
    tradFare = fare1 + fare2;
    jeepTime = Math.round((d1 + d2) * 5 + 12);

    const sign1 = JEEP_SIGNS[routeKey] || `"${hub.name.toUpperCase()}"`;
    const sign2 = JEEP_SIGNS[revKey]   || `"${toArea.name.toUpperCase()}"`;

    jeepSteps = [
      { icon:'📍', action:`Pumunta sa jeepney stop sa ${fromArea.name}`, detail:getLandmark(fromArea.name), cost:null },
      { icon:'🚏', action:`Sumakay ng Jeepney #1 papuntang ${hub.name}`, detail:`~${Math.round(d1 * 5 + 8)} minuto · Pumara ng kamay. Bayad: ₱${fare1.toFixed(2)} · Sabihin: "${hub.name} lang po"`, cost:`₱${fare1.toFixed(2)}`, signboard:sign1 },
      { icon:'🔀', action:`Bumaba sa ${hub.name} — mag-transfer dito`, detail:`Ito ang transfer point. Humanap ng bagong jeepney papuntang ${toArea.name}.`, cost:null },
      { icon:'🚏', action:`Sumakay ng Jeepney #2 papuntang ${toArea.name}`, detail:`~${Math.round(d2 * 5 + 8)} minuto · Pumara ng kamay. Bayad: ₱${fare2.toFixed(2)} · Sabihin: "${toArea.name} lang po"`, cost:`₱${fare2.toFixed(2)}`, signboard:sign2 },
      { icon:'📣', action:`Tumawag bago makarating`, detail:`"Manong, para po sa ${toArea.name}" — huwag kalimutang tumawag`, cost:null },
    ];
  } else {
    // ── Single jeepney ride ──
    tradFare = calcFare(ROAD_MODES[0], dist);
    jeepTime = Math.round(dist * 5 + 8);
    const signboard = JEEP_SIGNS[routeKey] || JEEP_SIGNS[revKey] || `"${toArea.name.toUpperCase()}"`;

    jeepSteps = [
      { icon:'📍', action:`Pumunta sa jeepney stop sa ${fromArea.name}`, detail:getLandmark(fromArea.name), cost:null },
      { icon:'🚏', action:`Humanap ng jeepney na may pabalang:`, detail:`Biyahe: ~${jeepTime} minuto · Pumara ng kamay — ilagay sa harap ng dibdib para mahinto`, cost:null, signboard },
      { icon:'💰', action:`Bayad: ₱${tradFare.toFixed(2)}`, detail:`Kung may sukli — hintayin ang tama. Huwag pabayaang manghingi ng dagdag.`, cost:`₱${tradFare.toFixed(2)}` },
      { icon:'📣', action:`Sabihin kung saan bababa`, detail:`"Manong, para po sa ${toArea.name}" — tumawag bago pa man makarating`, cost:null },
    ];
  }

  const busAcFare = calcTotalRoadFare(ROAD_MODES[3], fromArea, toArea);
  const jeepLabel = hub ? `Jeepney × 2 (via ${hub.name})` : 'Jeepney';
  const jeepNote  = hub ? `2 sakay · transfer sa ${hub.name}` : '1 sakay direkta';
  ...
  options.push({
    ...
    studentPrice: `₱${(tradFare * 0.8).toFixed(2)}`,
    time:`~${jeepTime} minuto · ${jeepNote}`, steps:jeepSteps
  });
  ...
  if (dist >= 6) {
    ...
    if (hub) {
      const fare1 = calcFare(ROAD_MODES[3], roadDist(fromArea, hub));
      const fare2 = calcFare(ROAD_MODES[3], roadDist(hub, toArea));
      busSteps = [
        { icon:'📍', action:`Pumunta sa bus stop sa ${fromArea.name}`, detail:getBusLandmark(fromArea.name), cost:null },
        { icon:'🚍', action:`Sumakay ng Aircon Bus papuntang ${hub.name}`, detail:`Biyahe: ~${Math.round(roadDist(fromArea, hub)*4+10)} minuto · Bayad: ₱${fare1.toFixed(2)}`, cost:`₱${fare1.toFixed(2)}` },
        { icon:'🔀', action:`Bumaba sa ${hub.name} — mag-transfer dito`, detail:`Humanap ng susunod na bus papuntang ${toArea.name}`, cost:null },
        { icon:'🚍', action:`Sumakay ng Aircon Bus papuntang ${toArea.name}`, detail:`Biyahe: ~${Math.round(roadDist(hub, toArea)*4+10)} minuto · Bayad: ₱${fare2.toFixed(2)}`, cost:`₱${fare2.toFixed(2)}` },
        { icon:'📣', action:`Sabihin kung saan bababa`, detail:`"Para po sa ${toArea.name}"`, cost:null },
      ];
    } else {
      busSteps = [
        { icon:'📍', action:`Pumunta sa bus stop sa ${fromArea.name}`, detail:getBusLandmark(fromArea.name), cost:null },
        { icon:'🚍', action:`Sumakay ng Aircon Bus papuntang ${toArea.name}`, detail:`Biyahe: ~${Math.round(jeepTime*1.15)} minuto · Hanapin ang bus na may tamang destinasyon sa harap`, cost:null },
    ...
    const isEdsaRoute = typeof EDSA_AREAS !== 'undefined' && EDSA_AREAS.includes(fromArea.name.toLowerCase()) && EDSA_AREAS.includes(toArea.name.toLowerCase());
    const busLabel = isEdsaRoute && !hub ? 'EDSA Carousel (Busway)' : hub ? `Aircon Bus × 2 (via ${hub.name})` : 'Aircon Bus';
    const busNote  = isEdsaRoute && !hub ? '1 sakay · EDSA dedicated lane' : hub ? `2 sakay · transfer sa ${hub.name}` : '1 sakay direkta';
    const timeMult = isEdsaRoute && !hub ? 0.7 : 1.15;

    options.push({
      rank:options.length+1, tag:'blue', tagLabel: isEdsaRoute && !hub ? 'Mabilis na Bus' : 'Mas Komportable',
      type: busLabel,
      price:`₱${busAcFare.toFixed(2)}`, priceClass:'blue',
      studentPrice: `₱${(busAcFare * 0.8).toFixed(2)}`,
      time:`~${Math.round(jeepTime*timeMult)} minuto · ${busNote}`,
      steps: busSteps
    });
  }
  ...
  options.forEach((o, i) => {
    ...
    if (i === 0) {
      o.tag = 'green';
      o.priceClass = 'green';
      o.tagLabel = o.type.includes('LRT') || o.type.includes('MRT') ? 'Pinaka-Mura at Mabilis' : 'Pinaka-Mura';
    } else {
      if (o.type.includes('LRT') || o.type.includes('MRT')) {
        o.tag = 'blue';
        o.priceClass = 'blue';
        o.tagLabel = 'Mabilis na Alternatibo';
      } else if (o.type.includes('Aircon')) {
        o.tag = 'blue';
        o.priceClass = 'blue';
        o.tagLabel = 'Mas Komportable';
      } else {
        o.tag = 'gray';
        o.priceClass = '';
        o.tagLabel = 'Alternatibo';
      }
    }
  });
    ...
    <div class="route-option ${i===0?'recommended':i===1?'runner-up':''}">
      <div class="ro-header ${i===0?'bg-green':i===1?'bg-blue':'bg-default'}">
        ...
        <div class="ro-price-block">
          ...
          ${opt.studentPrice ? `<div style="font-size:.8rem; font-weight:700; color:var(--green); margin-top:3px;">Estudyante ₱${opt.studentPrice.replace('₱','')}</div>` : ''}
...
function renderFares(fromArea, toArea, dist) {
  const isEdsaRoute = typeof EDSA_AREAS !== 'undefined' && EDSA_AREAS.includes(fromArea.name.toLowerCase()) && EDSA_AREAS.includes(toArea.name.toLowerCase());
  const activeModes = ROAD_MODES.filter(m => m.key !== 'carousel' || isEdsaRoute);

  const cheapestFare = Math.min(...activeModes.filter(m=>m.key!=='piston').map(m=>calcTotalRoadFare(m, fromArea, toArea)));

  const roadHtml = activeModes.map(m => {
    const fare = calcTotalRoadFare(m, fromArea, toArea);
    const rt   = fare * 2;
    ...
    return `
      <div class="fare-row ${isCheap?'cheapest':''} ${m.key==='piston'?'is-danger':''}">
        ...
        <div class="fare-prices">
          ...
          <div class="fare-rt" style="color:var(--green); font-weight:700;">Estudyante ₱${(fare * 0.8).toFixed(2)}</div>
    ...
    <div class="fare-row">
      ...
      <div class="fare-info">
        <div class="fare-name-row">
          ...
          <span class="fare-tag tag-discount">50% Bawas (Para sa Lahat)</span>
        </div>
      ...
      <div class="fare-prices">
        ...
        <div class="fare-rt" style="color:var(--green); font-weight:700;">Estudyante ₱${Math.round(r.disc[0]*0.8)}–${Math.round(r.disc[1]*0.8)}</div>
  ...
  document.getElementById('faresWrap').innerHTML =
    roadHtml +
    `<div class="rail-sep">🚇 LRT at MRT · 50% Diskuwento simula March 23, 2026 (Para sa lahat ng Mananakay)</div>` +
    railHtml;
}

function renderImpact(fromArea, toArea, dist) {
  const tradFare = calcTotalRoadFare(ROAD_MODES[0], fromArea, toArea);
  const pisFare  = calcTotalRoadFare(ROAD_MODES[5], fromArea, toArea);
...
function renderPiston(fromArea, toArea, dist) {
  const tradFare = calcTotalRoadFare(ROAD_MODES[0], fromArea, toArea);
  const pisFare  = calcTotalRoadFare(ROAD_MODES[5], fromArea, toArea);
    ...
    <div class="piston-card">
      ...
      <div>
        ...
        <div class="piston-body">
          Noong <strong>Abril 20, 2026</strong>, nag-file ang PISTON (samahan ng mga driver at operator) ng petisyon sa LTFRB para itaas ang base fare mula ₱13 patungong ₱23.<br/><br/>
          Para sa iyong ruta (${dist} km), ang pagkakaiba ay <span class="piston-highlight">+₱${diff.toFixed(2)} bawat sakay</span>.
          Ang <strong>₱13.00 na base fare ang tanging legal na pamantayan ngayon</strong>.
          Kung may driver na nagsingil ng ₱23 base fare — <strong>overcharging iyon at karapatang i-report o ireklamo</strong>.
        </div>
      </div>
    </div>`;
}
...
- Jeepney: ₱${tRadFare.toFixed(2)} one-way (₱13 base + ₱2/km pagkatapos ng 4km)
...
function analyze(fromArea, toArea) {
  ...
  renderFares(fromArea, toArea, dist);
  renderImpact(fromArea, toArea, dist);
  renderPiston(fromArea, toArea, dist);
